...
for review in texts_split:
    ...
    new_text = []
    for word in review:
        if (word[0] != '@') & ('http' not in word) & (not word.isdigit()):
...
features = add_pads(reviews_ints, seq_length=50)  # Увеличиваем длину обрабатываемого текста
...
embedding_dim = 100
hidden_dim = 128
n_layers = 3  # Увеличиваем количество слоев LSTM
model = LSTM_architecture(vocab_size, output_size, embedding_dim, hidden_dim, n_layers)
criterion = nn.BCELoss()
optimizer = torch.optim.AdamW(model.parameters(), lr=0.001)  # Используем AdamW
...
# Сохранение модели с weights_only=True
